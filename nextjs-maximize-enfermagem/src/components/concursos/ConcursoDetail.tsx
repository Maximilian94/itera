    ...
    <>
      ...
      <article className="mt-6 rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {concurso.examBoard && (
            <span
              className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-700 cursor-default"
              title={concurso.examBoard.name}
            >
              <AcademicCapIcon className="h-4 w-4 text-slate-500" />
              {concurso.examBoard.alias ?? concurso.examBoard.name}
            </span>
          )}
        </div>
