 ...
 * botão "Fazer o tour" (na dashboard) e as etapas guiadas em /concursos
 * (revisão de perfil + walkthrough da lista). Por viverem em componentes
 * diferentes, um `useState` local não serve — daí um store de módulo +
 ...
 * `reviewed`:  Passo 1 (revisão do perfil) já aceito nesta volta.
 * `listSeen`:  walkthrough progressivo da lista de concursos já concluído.
 *
 * `reviewed`/`listSeen` são resetados a cada `startTour` (o tour manual repete
 * do começo) e marcados ao avançar cada etapa.
 */
...
const LIST_KEY = 'maximize:onboarding:tour-list-seen'
...
function readLS(): TourState {
  try {
    ...
    return {
      ...
      listSeen: window.localStorage.getItem(LIST_KEY) === '1',
    }
  } catch {
    return { override: null, reviewed: false, listSeen: false }
  }
}
...
function commit(next: TourState) {
  ...
  try {
    ...
    flag(REVIEWED_KEY, next.reviewed)
    flag(LIST_KEY, next.listSeen)
  } catch {
...
/** Reinicia o tour do começo (revisão + walkthrough pendentes de novo). */
export function startTour() {
  commit({ override: 'active', reviewed: false, listSeen: false })
}
...
export function ackListTour() {
  if (!cache.listSeen) commit({ ...cache, listSeen: true })
}
export function dismissTour() {
  commit({ ...cache, override: 'dismissed' })
}
export function resetTour() {
  commit({ override: null, reviewed: false, listSeen: false })
}
...
export function getListSeen(): boolean {
  return cache.listSeen
}
...
const SERVER_SNAPSHOT: TourState = {
  override: null,
  reviewed: false,
  listSeen: false,
}
