...
const rowClasses = `
  ...
  cursor-pointer hover:bg-slate-100 p-2
`
...
function locationLabel(examBase: ExamBase) {
...
export function ExamBaseRow({ examBase }: ExamBaseRowProps) {
  const examBoardId = examBase.examBoardId
  const questionCount = examBase._count?.questions ?? 0
  const canNavigate = Boolean(examBoardId)
