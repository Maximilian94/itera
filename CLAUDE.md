...
- **Cargo com múltiplas provas (`cargoGroupId`):** um cargo pode ter N provas (tipos) com **bancos de questões distintos** — ex.: "Tipo 1"/"Tipo 2". Cada prova continua sendo um `ExamBase` próprio (questões/attempts/import/scoring/player **intactos**, tudo `examBaseId`-keyed); o agrupamento é leve: `ExamBase.cargoGroupId` (null = cargo standalone, o caso comum), `provaLabel` (rótulo) e `isPrimaryProva` (a prova representante — carrega ficha/edital/conteúdo programático/slug canônico; regra de serviço: 1 primária por grupo, garantida no `exam-base.service#update`). Helper `groupByCargo()` (`concurso.service.ts`) agrupa por `cargoGroupId ?? id`. Efeitos: `getConcursoDetail`/`listConcursos` emitem **1 card por grupo** (`questionCount` somado, `provaCount`, vagas/salário só do primário p/ não duplicar); `getCargoDetail` resolve o grupo, usa o primário como cargo e retorna `provas[]` (`{ examBaseId, slug, label, isPrimary, examDate, questionCount, userStats, studyPlan }`) — **um `studyPlan` por prova** (cada um sobre as questões da própria prova, ou as `previousExams` se vazia; corte é cargo-level do primário). Front: **não há mais seletor "Treinando para"**. A aba Treino do cargo é um **quadro de provas** (`ProvasBoard`) onde a prova primária vira a **prova oficial** (âncora) e as demais provas do cargo (+ relacionadas) viram **cards recomendados**; cada prova mantém seu `studyPlan`/treino próprio. Cargo de prova única → só a oficial, sem seção de recomendadas.
- **Provas para treinar / recomendação (relatedProvas):** o seletor do plano lista **opções de treino**, não só as provas do cargo. Regra temporal: **prova futura não tem questões próprias** → recomenda só **provas relacionadas**; **prova passada** → as próprias (com questões) **+ relacionadas**. "Relacionada" = mesmo cargo (`role`) com questões, fora do grupo: **tier 1** = mesma banca; **tier 2** = outra banca (ordenado tier1→tier2, depois examDate desc, cap 8). `getCargoDetail` retorna `relatedProvas[]` (`{ examBaseId, slug, institution, year, examBoardId, examBoardAlias, tier, questionCount, userStats, studyPlan }`) — cada uma com seu `studyPlan` próprio e a **própria `examBoardId`** (tier 2 é de outra banca → o player usa a banca da prova). Front: componentes em `web-react/src/features/concurso/components/treino/`. **A página do cargo tem 2 abas** (`CargoTabs`): **Detalhes** é a default (a ficha morfa vinda do concurso, ver transição abaixo) e **Treino** é a aba de ação. A aba **Treino** é um **quadro de provas** (`ProvasBoard`): a prova primária vira a **prova oficial** (âncora fixa no topo — se **futura** sem questões próprias, explica e recomenda equivalentes; se **passada**, mostra prontidão + "Treinar novamente"); as demais provas próprias com `questionCount>0` + `relatedProvas` viram **cards recomendados**, cada um com CTA Treinar/Continuar/Treinar novamente. Clicar "Treinar" numa prova abre o **treino em página cheia** **dentro do concurso** (`TrainingView`→`TrainingFlow`, barra de contexto + "Voltar às provas"): um **stepper horizontal** compacto (`StepperBar`) das 5 fases (Prova→Diagnóstico→Estudo→Re-tentativa→Final, `treino/-stages.config.ts`) e o **conteúdo da fase ocupa a página** (sem cartões colapsáveis). Fases embutidas em `embeds.tsx` (`DiagnosticoEmbed` = resultado+matérias+feedback IA; `EstudoListEmbed` = lista de pontos; `FinalEmbed` = progressão); na fase Estudo, abrir um ponto mostra `StudyItemFocus` (4 abas: Recomendação/Explicação/Exercícios/Questões que errei) com o stepper no topo. **Prova e Re-tentativa** (responder questões) abrem em tela cheia em `/treino`. Helper `useProgramActions(examBaseId, session)` centraliza começar/continuar; a sessão mais recente por prova vem de `useTrainingsQuery()` (`GET /training`, updatedAt desc → 1ª por `examBaseId`) e a aba Treino ganha selo "em andamento". **Aba Detalhes**: descrição da vaga, `SyllabusSection`, `SubjectBlock`/`SubjectDistribution`, `CompetitionSection`, `FichaCard` e `PreviousExamsCard`. Sem prova treinável → empty state com link `/treino`. **Removidos** na migração p/ full-page: `PlanPhases` (acordeão vertical), `NextStepCard`, `ReadinessStrip`, `SubjectMastery`, `OtherProvasList` e `CollapsibleCard`. **Shell consistente concurso↔cargo:** ambos usam **back-link** (o cargo não tem mais breadcrumb completo de 3 níveis) + header `[InstitutionMark] título + StatusPill inline`. **Transição concurso↔cargo (View Transitions API):** `view-transition-name` compartilhado na marca (`institution-mark`), título do concurso↔back-link (`concurso-heading`), nome do cargo do card clicado→h1 do cargo (`cargo-heading`, atribuído só no card clicado) e ficha (`ficha-card`, "Ficha do concurso"→"Ficha do cargo"); `viewTransition` nos `<Link>`, CSS em `styles.css`, progressive enhancement + `prefers-reduced-motion`. Mockups da direção: `web-react/design-mockups/{cargo-treino-fullpage,cargo-transition,cargo-provas-treinar}.html`. ⚠️ Começar treino consome a **cota de treino** (`training.service`: 1 grátis + limite mensal por plano). ⚠️ **Prova futura não deve ter questões próprias** (a prova ainda não foi aplicada) — semear questões numa prova futura gera estado inconsistente. Seed de demonstração: `api/prisma/seed-multi-prova.ts` (deixa `seed-c5-enfermeiro` futura sem questões e cria edições passadas Enfermeiro com questões; `--clean` reverte). Admin liga as provas no form de edição (`exams/editar/$examBaseId`): rótulo + cargoGroupId (botão "Gerar") + toggle "prova principal".
